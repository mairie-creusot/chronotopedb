...
torture-testé), pas encore audité en production.** Voir
...
| [`chronotope-server`](crates/chronotope-server) | Binaire HTTP (axum) exposant `ChronotopeEngine` — `/health` `/write` `/seal` `/read`. Seule frontière de confiance du dépôt. | 8 |

**117 tests** (`cargo test --workspace --all-targets`) + 1 doctest, tous
...
chaque ligne ci-dessous correspond à un test réel, pas à une intention.
...
- **Aucune persistance.** `ChronotopeEngine` est purement en mémoire — un
  ...
  compensé par une couche T1/T2 (hors périmètre de ce dépôt).
...
- **Aucune authentification/autorisation HTTP.** `chronotope-server` est
  documenté dans son propre code comme un harnais de test, pas le vrai
  transport envisagé (WebTransport, §3.3/§4) — n'importe quel client peut
  écrire dans n'importe quelle salle valide.
...
- **Pas de vrai réseau.** `chronotope-sim` prouve la logique de migration et
  de dégradation en mémoire ; il n'y a aucune implémentation de transport
  distribué, de consensus ou de réplication réseau dans ce dépôt.
- **H2 (coherence imperceptible) n'est pas mesurée ici** — elle exige des
...
- **Résidu connu, non éliminé :** un afflux extrême d'entités **distinctes**
  ...
  coût non strictement linéaire — le correctif de torture-test a réduit le
...
curl -X POST http://localhost:3200/write -H "Content-Type: application/json" \
  -d '{"room":0,"cell":0,"tick":0,"entity":1,"pos":[1.0,2.0,3.0],"yaw":0.5}'
# {"ok":true,"error":null}

curl -X POST http://localhost:3200/seal -H "Content-Type: application/json" \
...
# room/cell hors domaine (room < 65536, cell < 4096) : rejet propre, jamais
# transmis au moteur.
curl -i -X POST http://localhost:3200/write -H "Content-Type: application/json" \
  -d '{"room":99999,"cell":0,"tick":0,"entity":1,"pos":[0,0,0],"yaw":0}'
# HTTP/1.1 400 Bad Request
# {"ok":false,"error":"room ou cell hors domaine (room < 65536, cell < 4096)"}
```
...
docker run -p 3200:3200 ghcr.io/mairie-creusot/chronotopedb:latest
```
...
- `RUST_LOG` configurable (ex. `RUST_LOG=chronotope_core=trace`) — logs
  structurés `tracing`, jamais de `println!`.

Intégré dans le stack de dev de PawChat (`docker-compose.dev.yml`,
service `chronotope-dev`) en isolation réseau — aucun autre service ne le
consomme encore, c'est un banc de test.
...
```bash
cargo test --workspace --all-targets   # 117 tests
